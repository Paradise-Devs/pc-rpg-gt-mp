...
API.onServerEventTrigger.connect(function (event_name, args) {
    switch (event_name) {
        case "SetEditingHandle":
            ...
            editing_type_idx = 0;
            moving_speed_idx = 0;
            if (editing_handle != null)
                API.callNative("SET_ENTITY_COLLISION", editing_handle, false, true);
...
API.onKeyDown.connect(function (e, key) {
    if (editing_handle == null || API.isChatOpen())
        return;
    switch (key.KeyCode) {
        // Increase value
        case Keys.Up:
        ...
        case Keys.Right:
            editing_type_idx++;
            if (editing_type_idx >= editing_types.length)
                editing_type_idx = 0;
        ...
        case Keys.Left:
            editing_type_idx--;
            if (editing_type_idx < 0)
                editing_type_idx = editing_types.length - 1;
            API.displaySubtitle("Edit type: ~y~" + editing_types[editing_type_idx], 2000);
            break;
        // Moving speed - increase
        case Keys.Add:
            moving_speed_idx++;
            if (moving_speed_idx >= moving_speeds.length)
                moving_speed_idx = 0;
        ...
        case Keys.Subtract:
            moving_speed_idx--;
            if (moving_speed_idx < 0)
                moving_speed_idx = moving_speeds.length - 1;
            API.displaySubtitle("Moving speed: ~y~" + moving_speeds[moving_speed_idx], 2000);
            break;
        // Finish editing
        case Keys.Y:
